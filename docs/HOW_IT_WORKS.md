> - **Studio ↔ Orchestrator Proxy Bridge (2026-03-22)**: Pass `orchestrator_workflow_id` (+ optional `orchestrator_payload`, `orchestrator_timeout`, `orchestrator_wait_for_result`) in `user_metadata` to `handle_chat_message()` to run a saved DAG without the Studio LLM. **Default is async** (`POST /execute` only, instance id + poll URLs); set `orchestrator_wait_for_result` or `ORCHESTRATOR_BRIDGE_WAIT_FOR_RESULT=true` for blocking `run_and_wait`. Chat fields merge into the trigger when missing; optional `ORCHESTRATOR_API_TOKEN` for JWT orchestrator; sync mode returns structured HITL guidance when suspended. See `orchestrator/HOW_IT_WORKS.md` Step 17 and `tests/test_orchestrator_bridge.py`.
>
