> - **Orchestrator operator controls (2026-03-22)**: The visual hub supports cooperative **pause**, **cancel** (stop), and **resume** between DAG nodes (`POST …/pause`, `POST …/cancel`, `POST …/resume-paused`; DB migrations `0005`/`0006`). See `orchestrator/TECHNICAL_BLUEPRINT.md` §6.11 and `orchestrator/HOW_IT_WORKS.md` Step 6.
>
> - **Studio ↔ Orchestrator Proxy Bridge (2026-03-22)**: Pass `orchestrator_workflow_id` (+ optional `orchestrator_payload`, `orchestrator_timeout`, `orchestrator_wait_for_result`, `orchestrator_chat_reply_mode`, `orchestrator_include_context_json`) in `user_metadata` to `handle_chat_message()` to run a saved DAG without the Studio LLM. **Default is async** (`POST /execute` only, instance id + poll URLs); set `orchestrator_wait_for_result` or `ORCHESTRATOR_BRIDGE_WAIT_FOR_RESULT=true` for blocking `run_and_wait`. Sync completion uses **`orchestrator_user_reply`** from the hub when set (**Bridge User Reply** node); else friendly extraction; `ORCHESTRATOR_BRIDGE_CHAT_REPLY_MODE` / metadata overrides. Chat fields merge into the trigger when missing; optional `ORCHESTRATOR_API_TOKEN` for JWT orchestrator; sync mode returns structured HITL guidance when suspended. See `orchestrator/HOW_IT_WORKS.md` Step 17, `orchestrator/TECHNICAL_BLUEPRINT.md` §10, and `tests/test_orchestrator_bridge.py`.
>
...
4. **Shared MCP bridge + executor** (`tools/mcp_tools.py`, `mcp_server/tool_specs.py`):
   A shared `ThreadPoolExecutor(4)` is used for `_run_async`. In co-located mode, the main app consumes the same curated MCP tool spec registry as the standalone server; in remote mode, it discovers the same metadata over MCP `list_tools()` and executes via `call_tool()`.
